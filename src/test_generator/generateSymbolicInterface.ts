...
export function createInterfaceMockConstructor<ts_type>(interface_name:string, program_info:IProgramInfo<ts_type>){
    ...
    if(interface_properties_info !== undefined){
      Object.keys(interface_properties_info).forEach(function (property_name) {
        if(program_info.isInterfaceCallSignature(interface_properties_info[property_name])){
          var ret_function = generateSymbolicTypes.createSymbAssignment(interface_properties_info[property_name],program_info);
          stmts=stmts.concat(ret_function.stmts); 
          return_var = ret_function.var;
        } else {
      ...
      if(return_var){
        stmts.push(TsASTFunctions.generateReturnVar(return_var));
      }
    }
    return {
